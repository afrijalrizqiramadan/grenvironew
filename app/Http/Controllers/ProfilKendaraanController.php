...
class ProfilKendaraanController extends Controller
{
    ...
    public function detailPage($id): View {
        ...
        if($user->hasRole('administrator')) {
            ...
            $statuses = DeliveryStatus::join('customers', 'delivery_status.customer_id', '=', 'customers.id')
            ...
            $sensorData = Tracking::where('device_id', $id_device)
        ...
        $weatherData = json_decode($response->getBody(), true);
           ...
           $latestPressure = Tracking::where('device_id',  $id_device)
            ->orderBy('timestamp', 'desc')
            ->value('pressure');
        
            $latestTime = Tracking::where('device_id',  $id_device)
            ->orderBy('timestamp', 'desc')
            ->value('timestamp');
            $sensorData = Tracking::where('device_id', $id_device)
